...
function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:3001/api/login", { email, password }) // Updated endpoint
      .then((result) => {
        console.log(result);
        if (result.data.message === "Success") {
          localStorage.setItem("userId", result.data.userId);
          localStorage.setItem("userRole", result.data.role); // Store the role
          navigate("/workout");
        } else {
      ...
      .catch((err) => {
        ...
        alert("Login failed. Please try again later."); // Provide user feedback
      });
  };
